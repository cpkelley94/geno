...
for kP, cond in [(kP_vals[0], 'low'), (kP_vals[-1], 'high')]:
    ...
    # Breg_rtkT = np.zeros((len(rt_vals), len(kT_vals)))  # initialize
    # for i, rt_i in enumerate(rt_vals):
    #     for j, kT_j in enumerate(kT_vals):
    #         Breg_rtkT[i,j] = steady_state_Breg(r_t=rt_i, k_T=kT_j)
