...
def draw_images(img_list, titles=None, vmin=None, vmax=None, cmaps=None, out_name=None, bgcolor=None, dpi=600, **kwargs):
    """
    Convenience function to plot a list of 2D images.
    """
    ...
    for i, img in enumerate(img_list):
        if bgcolor:
            ax[i].set_facecolor(bgcolor)
        imxy.append(ax[i].imshow(img, vmin=vmin[i], vmax=vmax[i], cmap=cmaps[i], **kwargs))
    if titles:
        for i, title in enumerate(titles):
            ax[i].set_title(title)
    # plt.show()
...
cmap_NtoK = LinearSegmentedColormap.from_list('cmap_NtoK', ['#00000000', '#000000ff'])
